...
def synthesize_meta_from_data(job_id, raw):
    """v2.2 fallback: when build script has no META, derive a reasonable
    META dict from data.json's `project` and `totals` blocks. Same field
    names the rest of the script expects (units, total_fixtures, billed,
    retention, executed_co_count, etc.). Also synthesizes `pay_apps` from
    ar_invoices when present so the payApps array is populated."""
    p = raw.get('project', {}) or {}
    t = raw.get('totals', {}) or {}

    # Build pay_apps from ar_invoices if available. Sort by date then number,
    # compute running cumulative + percent-billed.
    pay_apps = []
    ari_sorted = sorted(raw.get('ar_invoices', []) or [], key=lambda x: (x.get('date', ''), x.get('invoice', '')))
    total_billed = sum((inv.get('amount', 0) or 0) for inv in ari_sorted) or 1
    cum = 0
    for i, inv in enumerate(ari_sorted, 1):
        amt = inv.get('amount', 0) or 0
        ret = -(inv.get('retainage', 0) or 0)
        cum += amt
        pct = round(cum / total_billed, 4)
        pay_apps.append([i, inv.get('date', ''), round(amt), round(ret), round(amt), round(cum), pct])

    return {
        'units': p.get('units', 1) or 1,
        'total_fixtures': p.get('total_fixtures', 0) or 0,
        'gc': p.get('general_contractor', ''),
        'billed': t.get('revenue_ar_actual', 0) or 0,
        'retention': t.get('retainage', 0) or 0,
        'orig_contract': t.get('contract_original', 0) or p.get('contract_original', 0) or 0,
        'rev_contract': t.get('contract_final', 0) or p.get('contract_final', 0) or 0,
        'executed_co_count': p.get('executed_co_count', 0) or 0,
        'cor_count': p.get('unexecuted_cor_count', 0) or 0,
        'rfi_count': p.get('rfi_count', 0) or 0,
        'submittal_count': p.get('submittal_count', 0) or 0,
        'co_dollar_impact': p.get('net_co_dollar_impact', 0) or 0,
        'duration_months': p.get('duration_months', 0) or 0,
        'change_events': [],   # v2.2 doesn't itemize per-CO descriptions
        'pay_apps': pay_apps,  # synthesized from ar_invoices above
    }
...
def load_data(job_id):
    ...
    # v2.0 schema uses `cost_code_summaries`, `line_items`, `worker_wages`.
    # v2.2 schema uses `cost_codes`, `ap_invoices`/`ar_invoices`, `workers`.
    # Normalize both shapes here so the rest of the script doesn't care.
    cc_key = 'cost_code_summaries' if raw.get('cost_code_summaries') else 'cost_codes'
    code_totals = {}
    for cs in raw.get(cc_key, []):
        if cs.get('code') is None:
            continue
        code_totals[cs['code']] = {
            'desc': cs.get('description', ''),
            'orig': cs.get('original_budget', 0) or 0,
            'rev': cs.get('current_budget', 0) or 0,
            'actual': cs.get('actual_amount', 0) or 0,
            'variance': cs.get('plus_minus_budget', 0) or 0,
            'net_due': cs.get('net_due', 0) or 0,
            'retainage': cs.get('retainage', 0) or 0,
            'reg_hours': cs.get('regular_hours', 0) or 0,
            'ot_hours': cs.get('overtime_hours', 0) or 0,
        }
    ...
    if raw.get('line_items'):
        ...
        # v2.2 path: synthesize transactions from ap_invoices + ar_invoices.
        # If those aren't present either, fall back to top-level vendors summary.
        for inv in raw.get('ap_invoices', []) or []:
            transactions.append({
                'src': 'AP',
                'party_name': inv.get('vendor', '') or inv.get('name', ''),
                'amount': inv.get('amount', 0) or 0,
                'hours': 0,
                'code': inv.get('cost_code', '') or inv.get('code', ''),
                'date': inv.get('date', ''),
                'ref': inv.get('invoice', '') or inv.get('reference', ''),
            })
        for inv in raw.get('ar_invoices', []) or []:
            transactions.append({
                'src': 'AR',
                'party_name': inv.get('customer', ''),
                'amount': inv.get('amount', 0) or 0,
                'hours': 0,
                'code': '',
                'date': inv.get('date', ''),
        ...
        if not transactions and raw.get('vendors'):
            # Last-resort fallback: synthesize a single AP transaction per vendor
            # so gen_all_vendors still produces output. Date/ref left blank.
            for v in raw['vendors']:
                transactions.append({
                    'src': 'AP',
                    'party_name': v.get('name', ''),
                    'amount': v.get('spend', 0) or 0,
                    'hours': 0,
                    'code': '',
                    'date': '',
                    'ref': '',
    ...
    # v2.0 uses `worker_wages`, v2.2 uses `workers`. Same field shape.
    worker_wages = raw.get('worker_wages') or raw.get('workers') or []
...
def main():
    ...
    for job_id in jobs:
        ...
        if not meta:
            # v2.2 fallback: if data.json has a 'project' block (v2.2 schema),
            # synthesize META from it. Otherwise use minimal defaults.
            if data.get('raw', {}).get('project'):
                meta = synthesize_meta_from_data(job_id, data['raw'])
                print(f"// {job_id}: META synthesized from v2.2 data.json (no META in build script)", file=sys.stderr)
            else:
