...
DATA_FILE = _HERE / '2052_data.json'
OUT_FILE = _HERE / 'OWP_2052_JCR_Cortex_v2.xlsx'
...
META_2052 = {
    ...
    'ri_foreman': None,
    'cos': [
        # CO log not yet captured from GDrive — JDR shows net +$97,035 (additive, +4.5%)
        # CO #2 referenced in Job Ticket: added duplex booster package
    ],
    'pay_apps': [],
    'change_log': [],
    'predictive_signals': [],
...
def generate_predictive_signals(data):
    ...
    # Retention
    ret = abs(META_2052.get('retention', 0))
    billed = abs(META_2052.get('billed', 0))
...
if __name__ == "__main__":
    data = load_data()
    META_2052['predictive_signals'] = generate_predictive_signals(data)
    out = build_workbook(dict(META_2052))
    print(f"Built: {out}")
