#!/usr/bin/env python3
"""
build_2105.py — Build the 17-tab JCR Cortex v2 workbook for Job 2105
(Compass, 3000 Western) — CANCELLED project.

Input:   2105_data.json  (from parse_2105.py)
Output:  OWP_2105_JCR_Cortex_v2.xlsx

Notes:
- CANCELLED projects use revised budget lens (same as live).
- Status shows "CANCELLED" instead of "Live / in-progress"
...
DATA_FILE = _HERE / '2105_data.json'
OUT_FILE = _HERE / 'OWP_2105_JCR_Cortex_v2.xlsx'
...
def tab_overview(wb, data, meta):
    ...
    ws["A1"].font = F_H1
    ws["A2"] = f"{meta['gc']} · {meta['units']} units · CANCELLED"
    ws["A2"].font = F_NOTE
...
def tab_job_info(wb, data, meta):
    ...
    rows = [
        ...
        ("Status", "CANCELLED"), ("JDR report date", meta['report_date']),
    ]
    r = 3
    for lbl, val in rows:
...
def tab_predictive(wb, data, meta):
    ...
    ws["A1"] = "Predictive Signals (cancelled project)"; ws["A1"].font = F_H1
...
def tab_change_log(wb, data, meta):
    ...
    ws.cell(row=4, column=2, value="v1.0 · cancelled").font = F_BODY
    ws.cell(row=4, column=3, value=f"Initial build for #{meta['job_id']} cancelled project. 17-tab JCR schema on revised budget basis.").font = F_BODY
...
META_2105 = {
    'job_id': '2105',
    'name': 'Compass, 3000 Western',
    'short_name': '3000 Western',
    'gc': 'Compass General Construction 1, LLC',
    'owner': '3000 Western LLC',
    'units': 219,
    'total_fixtures': 0,
    'insurance': 'Standard COI',
    'job_start': 'N/A (cancelled)',
    'expected_completion': 'CANCELLED',
    'architect': 'Encore Architects',
    'mep': 'GLUMAC / Franklin Engineering',
    'location': '3000 Western Ave, Seattle, WA',
    'project_type': 'Living Building Challenge (LBC) multifamily (cancelled)',
    'design_contract': 124490,
    ...
    'pay_apps': [],
    'change_log': [
        {'date': 'Pending', 'type': 'COR', 'ref': 'COR#01', 'desc': 'Pending — $24,995 (not executed, project cancelled)', 'amount': 24995},
    ],
    'predictive_signals': [],
}
...
def generate_predictive_signals(data):
    ...
    # Retention
    ret = abs(META_2105.get('retention', 0))
    billed = abs(META_2105.get('billed', 0))
    ret_pct = ret / billed if billed else 0
    signals.append((
        'Retention aging',
        '✓ OK' if ret_pct < 0.06 else '⚠ WATCH',
        f'${ret:,.0f} held · {ret_pct:.1%} of billed · zero (cancelled)'
    ))
...
if __name__ == "__main__":
    data = load_data()
    META_2105['predictive_signals'] = generate_predictive_signals(data)
    out = build_workbook(dict(META_2105))
    print(f"Built: {out}")
