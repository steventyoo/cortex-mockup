#!/usr/bin/env python3
"""
notionize_2115.py — Build the Notion-styled 2-tab summary workbook for
Job 2115 (Holland, Ballard Blossom) — live project (pre-construction / DD).

Input:   2115_data.json  (from parse_2115.py)
Output:  OWP_2115_JCR_Summary_Notion_v2.xlsx
"""
import json
...
OUT_FILE = _HERE / 'OWP_2115_JCR_Summary_Notion_v2.xlsx'
...
def build_summary_tab(wb, data, meta):
    ...
    ws["A2"] = f"{meta['gc']} · {meta['units']} units · ACTIVE · {meta['pct_complete']:.1%} billed"
...
def generate_predictive_signals(data):
    ...
    signals.append(('Retention', '✓ OK', f'${344992.64:,.0f} held · 5% standard'))
...
META_2115 = {
    ...
    'units': 1,                              # TBD — placeholder (avoids div-by-zero in Notion render)
    'billed': 0.0,
    'retention': 0.0,
}
...
if __name__ == "__main__":
    ...
    out = build_notion_workbook(dict(META_2115))
    print(f"Built: {out}")
