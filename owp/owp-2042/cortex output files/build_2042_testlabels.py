#!/usr/bin/env python3
"""
build_2042_testlabels.py — Build OWP_2042_JCR_Test_Labels.xlsx
...
Consumes /sessions/gracious-relaxed-pascal/2042_data.json produced by parse_2042.py.
"""
import json
...
OUT_FILE = OUT_DIR / 'OWP_2042_JCR_Test_Labels.xlsx'
...
def build_report_record(wb, d):
    ...
    write_title(ws, 'JCR TEST LABELS — JOB 2042',
...
def build_reconciliation_log(wb, d):
    ...
    write_title(ws, 'RECONCILIATION LOG — Job 2042',
                'Worked example of the Reconciliation Protocol running against Job 2042 (JCR-only — no external cross-checks). Each row shows the JCR-derived value, the cross-check value, and the observed status. Protocol contract lives in JCR Schema v4 → Reconciliation Protocol tab.')
    ws.append([])
    ...
    rows = [
        ...
        ('RC-06', 'scope_metrics[] / scope_benchmarks', 'pending', '—',
         ...
         'Awaiting plumbing scope-metric declarations for Job 2042 (e.g., fixture count, DFU total, linear feet of main). Labeler should not emit scope_benchmarks until scope_metrics is populated.'),
    ...
    ws.append([])
    ws.append(['How to use: when a labeler produces a new set of derived values for Job 2042, update columns C and D in the rows above and set status per the Protocol contract.'])
