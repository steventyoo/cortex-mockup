#!/usr/bin/env python3
"""
build_2042.py — Build OWP_2042_JCR_Cortex_v2.xlsx (17 tabs)

Greenfield 17-tab JCR for Job 2042 Shelter Holdings Metro Line Flats.
...
Reads /sessions/gracious-relaxed-pascal/2042_data.json from parse_2042.py.
...
DATA_JSON = Path('/sessions/gracious-relaxed-pascal/2042_data.json')
ENRICHED_META_JSON = Path('/sessions/gracious-relaxed-pascal/2042_enriched_meta.json')
OUT_DIR = Path('/sessions/gracious-relaxed-pascal/mnt/cortex-mockup/owp/owp-2042/cortex output files')
OUT_DIR.mkdir(parents=True, exist_ok=True)
OUT_FILE = OUT_DIR / 'OWP_2042_JCR_Cortex_v2.xlsx'
...
def build_overview(wb, d):
    ...
    title(ws, 'OWP JOB 2042 — SHELTER HOLDINGS METRO LINE FLATS')
...
def build_job_info(wb, d):
    ...
    title(ws, 'JOB INFORMATION — 2042')
    caption(ws, 'Project identity + metadata enriched from GDrive close-out package (Metro Line Flats project info PDF, signed subcontract, permits, scope).')
    ws.append([])
    hdr_row(ws, 3, ['Field', 'Value', 'Source / Notes'])
    rows = [
        ('job_number',         rr['job_number'],                                          'JDR cover page'),
        ('job_name',           em.get('job_name') or rr['job_name'],                      'Metro Line Flats project info PDF'),
        ('job_name_long',      em.get('job_name_long', rr['job_name']),                   'Metro Line Flats project info PDF'),
        ('general_contractor', em.get('general_contractor'),                              'Signed subcontract SC-53799-027 — Shelter Holdings / Boren and Spruce, LLC'),
        ('owner',              em.get('owner'),                                           'Signed subcontract §A.a — Boren and Spruce, LLC'),
        ('architect',          em.get('architect'),                                       'Contract counterparty: Boren and Spruce, LLC'),
        ...
        ('permit_jurisdiction', em.get('permit_jurisdiction'),                            'Seattle DCI permit SR1402734'),
        ('permits_issued',     '; '.join(em.get('permits_issued', [])),                   'Permits folder: 3 permit types × renewals/extensions'),
        ('status',             'CLOSED',                                                  'Inferred from JDR filter: "Job 2042 only, Open, Closed"'),
...
def build_change_orders(wb, d):
    ...
    title(ws, 'CHANGE ORDER LOG — 2042')
...
def build_project_team(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'PROJECT TEAM — 2042')
    caption(ws, 'Primary contacts extracted from Metro Line Flats project start-up PDF + signed subcontract.')
    ws.append([])
...
def build_document_pipeline(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'DOCUMENT PIPELINE — 2042')
    caption(ws, 'Quantitative document counts from the Metro Line Flats GDrive close-out package — used by the Cortex dashboard for CO/RFI/Submittal density metrics.')
    ws.append([])
...
def build_contract(wb, d):
    ...
    title(ws, 'CONTRACT SUMMARY — 2042')
...
def build_cost_code_detail(wb, d):
    ...
    title(ws, 'COST CODE DETAIL — 2042')
...
def build_wage_tiers(wb, d):
    ...
    workers = d['worker_wages']
    df = d['derived_fields']
    title(ws, 'WAGE TIERS & PERCENTILES — 2042')
...
def build_productivity(wb, d):
    ...
    title(ws, 'PRODUCTIVITY METRICS — 2042')
    ...
    rows = [
        ...
        ('Units (residential)',      units,                                            'Metro Line Flats scope doc: 68 standard + 4 Type A ADA'),
...
def build_benchmarks(wb, d):
    ...
    df = d['derived_fields']
    title(ws, 'BENCHMARKS & DERIVED FIELDS — 2042')
...
def build_predictive(wb, d):
    ...
    df = d['derived_fields']
    cs = d['cost_code_summaries']
    title(ws, 'PREDICTIVE SIGNALS — 2042')
...
def build_reconciliation_log(wb, d):
    ...
    title(ws, 'RECONCILIATION LOG — JOB 2042')
    caption(ws, 'Worked example of the Reconciliation Protocol running against Job 2042 (JCR-only, no external cross-checks). Mirrors Sam Test-Labels v4 Reconciliation Log tab.')
    ws.append([])
    ...
    rows = [
        ...
        ('RC-06', 'scope_metrics / scope_benchmarks', 'pending', '—',
         ...
         'Awaiting plumbing scope-metric declarations for Job 2042 (fixture count, DFU total, linear feet of main). Same status as Job 2020.'),
