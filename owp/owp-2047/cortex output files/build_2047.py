#!/usr/bin/env python3
"""
build_2047.py — Build OWP_2047_JCR_Cortex_v2.xlsx (17 tabs)

Greenfield 17-tab JCR for Job 2047 GRE Greenlake.
...
Reads /sessions/gracious-relaxed-pascal/2047_data.json from parse_2047.py.
...
DATA_JSON = Path('/sessions/gracious-relaxed-pascal/2047_data.json')
ENRICHED_META_JSON = Path('/sessions/gracious-relaxed-pascal/2047_enriched_meta.json')
OUT_DIR = Path('/sessions/gracious-relaxed-pascal/mnt/cortex-mockup/owp/owp-2047/cortex output files')
OUT_DIR.mkdir(parents=True, exist_ok=True)
OUT_FILE = OUT_DIR / 'OWP_2047_JCR_Cortex_v2.xlsx'
...
def build_overview(wb, d):
    ...
    title(ws, 'OWP JOB 2047 — GRE GREENLAKE')
...
def build_job_info(wb, d):
    ...
    title(ws, 'JOB INFORMATION — 2047')
    caption(ws, 'Project identity + metadata enriched from GDrive close-out package (GRE Greenlake Apartments project info PDF, signed subcontract, permits, scope).')
    ws.append([])
    hdr_row(ws, 3, ['Field', 'Value', 'Source / Notes'])
    rows = [
        ('job_number',         rr['job_number'],                                          'JDR cover page'),
        ('job_name',           em.get('job_name') or rr['job_name'],                      'GRE Greenlake Apartments project info PDF'),
        ('job_name_long',      em.get('job_name_long', rr['job_name']),                   'GRE Greenlake Apartments project info PDF'),
        ('general_contractor', em.get('general_contractor'),                              'Signed subcontract 7926 — Shelter Holdings / GRE Green Lake LLC'),
        ('owner',              em.get('owner'),                                           'Signed subcontract §A.a — GRE Green Lake LLC'),
        ('architect',          em.get('architect'),                                       'Contract counterparty: GRE Green Lake LLC'),
        ...
        ('contract_signed',    em.get('contract_signed_date'),                            'Signed subcontract — Mike Donelson 9/11/17'),
        ...
        ('status',             'CLOSED',                                                  'Inferred from JDR filter: "Job 2047 only, Open, Closed"'),
...
def build_change_orders(wb, d):
    ...
    title(ws, 'CHANGE ORDER LOG — 2047')
...
def build_project_team(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'PROJECT TEAM — 2047')
    caption(ws, 'Primary contacts extracted from GRE Greenlake Apartments project start-up PDF + signed subcontract.')
    ws.append([])
...
def build_document_pipeline(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'DOCUMENT PIPELINE — 2047')
    caption(ws, 'Quantitative document counts from the GRE Greenlake Apartments GDrive close-out package — used by the Cortex dashboard for CO/RFI/Submittal density metrics.')
    ws.append([])
    ...
    rows = [
        ...
        ('Backcharges',                  em.get('backcharge_count'),        'Change Orders/COR_s/ (no backcharges logged for GRE Greenlake)'),
    ]
...
def build_contract(wb, d):
    ...
    title(ws, 'CONTRACT SUMMARY — 2047')
...
def build_cost_code_detail(wb, d):
    ...
    title(ws, 'COST CODE DETAIL — 2047')
...
def build_wage_tiers(wb, d):
    ...
    workers = d['worker_wages']
    df = d['derived_fields']
    title(ws, 'WAGE TIERS & PERCENTILES — 2047')
...
def build_productivity(wb, d):
    ...
    title(ws, 'PRODUCTIVITY METRICS — 2047')
    ...
    rows = [
        ...
        ('Units (residential)',      units,                                            'GRE Greenlake Apartments scope doc: 61 Type B + 11 Type A ADA'),
...
def build_benchmarks(wb, d):
    ...
    df = d['derived_fields']
    title(ws, 'BENCHMARKS & DERIVED FIELDS — 2047')
...
def build_predictive(wb, d):
    ...
    df = d['derived_fields']
    cs = d['cost_code_summaries']
    title(ws, 'PREDICTIVE SIGNALS — 2047')
...
def build_reconciliation_log(wb, d):
    ...
    title(ws, 'RECONCILIATION LOG — JOB 2047')
    caption(ws, 'Worked example of the Reconciliation Protocol running against Job 2047 (JCR-only, no external cross-checks). Mirrors Sam Test-Labels v4 Reconciliation Log tab.')
    ws.append([])
    ...
    rows = [
        ...
        ('RC-06', 'scope_metrics / scope_benchmarks', 'pending', '—',
         ...
         'Awaiting plumbing scope-metric declarations for Job 2047 (fixture count, DFU total, linear feet of main). Same status as Job 2020.'),
