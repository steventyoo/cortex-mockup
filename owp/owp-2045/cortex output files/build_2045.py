#!/usr/bin/env python3
"""
build_2045.py — Build OWP_2045_JCR_Cortex_v2.xlsx (17 tabs)

Greenfield 17-tab JCR for Job 2045 GRE Notch Apartments.
...
Reads /sessions/gracious-relaxed-pascal/2045_data.json from parse_2045.py.
...
DATA_JSON = Path('/sessions/gracious-relaxed-pascal/2045_data.json')
ENRICHED_META_JSON = Path('/sessions/gracious-relaxed-pascal/2045_enriched_meta.json')
OUT_DIR = Path('/sessions/gracious-relaxed-pascal/mnt/cortex-mockup/owp/owp-2045/cortex output files')
OUT_DIR.mkdir(parents=True, exist_ok=True)
OUT_FILE = OUT_DIR / 'OWP_2045_JCR_Cortex_v2.xlsx'
...
def build_overview(wb, d):
    ...
    title(ws, 'OWP JOB 2045 — GRE NOTCH APARTMENTS')
...
def build_job_info(wb, d):
    ...
    title(ws, 'JOB INFORMATION — 2045')
    caption(ws, 'Project identity + metadata enriched from GDrive close-out package (Notch Apartments project info PDF, signed subcontract, permits, scope).')
    ws.append([])
    hdr_row(ws, 3, ['Field', 'Value', 'Source / Notes'])
    rows = [
        ('job_number',         rr['job_number'],                                          'JDR cover page'),
        ('job_name',           em.get('job_name') or rr['job_name'],                      'Notch Apartments project info PDF'),
        ('job_name_long',      em.get('job_name_long', rr['job_name']),                   'Notch Apartments project info PDF'),
        ('general_contractor', em.get('general_contractor'),                              'Signed subcontract 7927 — Shelter Holdings / Newcastle II LLC'),
        ('owner',              em.get('owner'),                                           'Signed subcontract §A.a — Newcastle II LLC'),
        ('architect',          em.get('architect'),                                       'Contract counterparty: Newcastle II LLC'),
        ('location',           em.get('location'),                                        'Signed subcontract + permit records'),
        ('project_type',       em.get('project_type'),                                    'Scope doc + submittal TOC (6-story mixed-use + 2 parking levels)'),
        ('unit_count',         em.get('units'),                                           'Plan drawings: 6-story, 187 units (studios, open 1-BR, 1-BR, 2-BR, 4 Type A ADA)'),
        ...
        ('contract_signed',    em.get('contract_signed_date'),                            'Signed subcontract — Mike Donelson 10/02/17'),
        ('permit_jurisdiction', em.get('permit_jurisdiction'),                            'City of Newcastle permit (Notch plumbing + fixture count)'),
        ('permits_issued',     '; '.join(em.get('permits_issued', [])),                   'Permits folder: 3 permit types × renewals/extensions'),
        ('status',             'CLOSED',                                                  'Inferred from JDR filter: "Job 2045 only, Open, Closed"'),
...
def build_change_orders(wb, d):
    ...
    title(ws, 'CHANGE ORDER LOG — 2045')
...
def build_project_team(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'PROJECT TEAM — 2045')
    caption(ws, 'Primary contacts extracted from Notch Apartments project start-up PDF + signed subcontract.')
    ws.append([])
...
def build_document_pipeline(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'DOCUMENT PIPELINE — 2045')
    caption(ws, 'Quantitative document counts from the Notch Apartments GDrive close-out package — used by the Cortex dashboard for CO/RFI/Submittal density metrics.')
    ws.append([])
    ...
    rows = [
        ...
        ('Permits issued',               len(em.get('permits_issued', [])), 'Permits/ (plumbing permit application + inspection requests)'),
        ('Backcharges',                  em.get('backcharge_count'),        'Change Orders/COR_s/ (no backcharges logged for Notch)'),
    ]
...
def build_contract(wb, d):
    ...
    title(ws, 'CONTRACT SUMMARY — 2045')
...
def build_cost_code_detail(wb, d):
    ...
    title(ws, 'COST CODE DETAIL — 2045')
...
def build_wage_tiers(wb, d):
    ...
    workers = d['worker_wages']
    df = d['derived_fields']
    title(ws, 'WAGE TIERS & PERCENTILES — 2045')
...
def build_productivity(wb, d):
    ...
    title(ws, 'PRODUCTIVITY METRICS — 2045')
    ...
    rows = [
        ...
        ('Units (residential)',      units,                                            'Notch Apartments scope doc: 179 Type B + 8 Type A ADA'),
...
def build_benchmarks(wb, d):
    ...
    df = d['derived_fields']
    title(ws, 'BENCHMARKS & DERIVED FIELDS — 2045')
...
def build_predictive(wb, d):
    ...
    df = d['derived_fields']
    cs = d['cost_code_summaries']
    title(ws, 'PREDICTIVE SIGNALS — 2045')
...
def build_reconciliation_log(wb, d):
    ...
    title(ws, 'RECONCILIATION LOG — JOB 2045')
    caption(ws, 'Worked example of the Reconciliation Protocol running against Job 2045 (JCR-only, no external cross-checks). Mirrors Sam Test-Labels v4 Reconciliation Log tab.')
    ws.append([])
    ...
    rows = [
        ...
        ('RC-06', 'scope_metrics / scope_benchmarks', 'pending', '—',
         ...
         'Awaiting plumbing scope-metric declarations for Job 2045 (fixture count, DFU total, linear feet of main). Same status as Job 2020.'),
