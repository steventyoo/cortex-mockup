#!/usr/bin/env python3
"""
build_2118.py — Build the 17-tab JCR Cortex v2 workbook for Job 2118
(Holland, Ballard Blossom) — live/active project (pre-construction / DD phase).

Input:   2118_data.json  (from parse_2118.py)
Output:  OWP_2118_JCR_Cortex_v2.xlsx
...
DATA_FILE = _HERE / '2118_data.json'
OUT_FILE = _HERE / 'OWP_2118_JCR_Cortex_v2.xlsx'
...
META_2118 = {
    'job_id': '2118',
    'name': 'Edmonds Behar Apartments',
    'short_name': 'Edmonds Behar',
    'gc': 'Exxel Pacific, Inc.',
    'owner': None,
    'units': 409,                           # Per Jan 16 2026 proposal (465 Type B + 21 Type A toilets = 486 bathrooms across ~409 dwellings)
    'total_fixtures': None,
    'insurance': 'Standard (COI)',
    'job_start': 'TBD',                     # No Sage activity yet
    'expected_completion': 'TBD — schedule not published',
    'architect': None,
    'mep': 'Robison Engineering',           # Design contract signed Mar 16 2026 ($125k)
    'location': 'Edmonds, WA',
    'project_type': '6-story multifamily, 2 towers, 3 below-grade levels (409 units)',
    'subcontract_number': None,
    ...
    'ri_foreman': None,
    'trim_foreman': None,
    'gc_pm': 'Tommy Booth',                 # Exxel PM (same as #2017 Mack Urban + #2022)
    'gc_pe': None,
    'owp_vp': 'Richard Donelson',
    'previous_plumber': None,
    'invoice_count': None,
    'vendor_quote_count': None,
    # Bid-phase budget intelligence (from Jan 16 2026 proposal pricing breakdown)
    ...
    'gmp_set_date': '2024-11-27',
    'bid_history_months': 22,               # First bid Mar 2024 → Jan 2026
    'water_heating_system': 'Gas (6x Bock OT-800N-A + 2x OT-150N)',
    'status_note': 'No JDR yet — bidding/design phase. Construction subcontract pending.',
    'cos': [
...
def generate_predictive_signals(data):
    ...
    # Retention
    ret = abs(META_2118.get('retention', 0))
    billed = abs(META_2118.get('billed', 0))
...
if __name__ == "__main__":
    data = load_data()
    META_2118['predictive_signals'] = generate_predictive_signals(data)
    out = build_workbook(dict(META_2118))
    print(f"Built: {out}")
