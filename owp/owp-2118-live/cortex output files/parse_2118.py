#!/usr/bin/env python3
"""
parse_2118.py — Parse Job 2118 JDR PDF → 2118_data.json

NOTE: No JDR PDF exists yet for 2118 as of the Apr 3 2026 Sage export.
The job is in bidding/design phase only, not yet in Sage. Script is
provided for consistency with other live projects and will run the
moment a JDR is published in owp/Gemini Accounting Reports/. Until
then, parse returns an empty data set (zero line_items, $0 actuals).

Project: 2118 (Exxel Pacific, Edmonds Behar Apartments) — 6-story,
2-tower, 409-unit apartment in Edmonds, WA, with 3 below-grade levels.
Long bid history since Mar 2024; latest Jan 16 2026 OWP plumbing
proposal at $7.18M total plumbing budget. Robison Engineering design
contract signed Mar 16 2026 ($125K).

Customer code: 2118EX · Client: Exxel Pacific, Inc. · GC: Exxel
(Tommy Booth, PM — same as #2017 Mack Urban + #2022).
"""
import json
import re
...
# owp-2118-live/cortex output files/ → owp/ → Gemini Accounting Reports/
PDF = _HERE.parent.parent / 'Gemini Accounting Reports' / '2118 Job Detail Report.pdf'
OUT = _HERE / '2118_data.json'

PROJECT_META = {
    'job_number': '2118',
    'job_name': 'Edmonds Behar Apartments',
    'general_contractor': 'Exxel Pacific, Inc.',
    'owner': None,
    'location': 'Edmonds, WA',
    'project_type': '6-story multifamily, 2 towers, 3 below-grade levels (409 units)',
    'units': 409,
    'total_fixtures': None,
    'architect': None,
    'mep_engineer': 'Robison Engineering (Jeff MacGillivray, Mechanical Designer)',
    'report_date': '2026-04-03',
    'contract_original': 7183400,         # Total plumbing budget from Jan 16 2026 proposal
    'total_cos_net': 0,
    'contract_final': 7183400,
    'executed_co_count': 0,
    'cor_count': 0,
    'cancelled_cor_count': 0,
    'rfi_count': 0,
    'asi_count': 0,
    'submittal_count': 0,
    'total_pos': 0,
    ...
    'start_date': None,                   # No Sage activity yet
    'end_date': None,
    'retention_rate': 0.05,
    'retention_amount': 0.0,
    'billed_to_date': 0.0,
    'base_bid': 7183400,
    'bid_date': '2026-01-16',             # Latest OWP plumbing proposal to Exxel (Tommy Booth)
    'bid_labor': None,
    'bid_material': None,
    'bid_subcontractor_other': None,
    'bid_overhead_profit_pct': None,
    'bid_total_hours': None,
    'bid_avg_wage': None,
    # Budget breakdown from Jan 16 2026 proposal
    'design_fee_owp': 173800,             # 2-D plumbing design & engineering
    'permit_fee_budget': 77500,           # City of Edmonds plumbing/gas plan review & permits
    'construction_budget': 6325300,       # Plumbing construction
    'fixture_budget': 606800,             # Residential + common area fixture budget
    'robison_design_contract': 125000,    # Signed Mar 16 2026 ($120k design + $5k CA)
    'insurance': 'Standard (COI)',
    'tco_date': None,
    'gc_pm': 'Tommy Booth',               # Exxel PM
    'gc_pe': None,
    'gc_project_number': None,
    'ri_foreman': None,
    'trim_foreman': None,
    'owp_vp': 'Richard Donelson',
    'previous_plumber': None,
    'invoice_count': None,
    'vendor_quote_count': None,
    'gmp_set_date': '2024-11-27',         # Per Robison fee proposal
    'bid_history_months': 22,             # First bid Mar 2024 → latest Jan 2026
    'water_heating_system': 'Gas (6x Bock OT-800N-A 800mbh + 2x OT-150N 150mbh)',
    'status_note': 'No JDR yet — bidding/design phase. Design contract signed Mar 16 2026 with Robison. Construction subcontract pending.',
}
...
CAT.update({'011': 'Labor', '039': 'Material', '995': 'Burden', '998': 'Burden', '999': 'Revenue'})
...
CODE_HDR_RE = re.compile(r'^(\d{3})\s+([A-Z][A-Za-z &/\-.,()]+)$')
PAGE_HDR_MARKERS = (
    'OWP, LLC', 'Job Detail Report', 'for Job "2118"', 'Job Number: 2118',
    'Job Name: Exxel, Edmonds Behar', 'Legal:',
        ...
        if m:
            ...
            if i + 1 < len(clean):
                ...
                if hm:
                    ...
                    i += 1
            # Only append if we have a valid current_code (skip phantom entries)
...
def build_reconciliation(line_items, code_summaries):
    """Reconcile parsed line items against PDF's own Cost Code Totals.

    The PDF cost code totals are AUTHORITATIVE (printed by Sage). The parsed
    line items may miss some transactions due to format edge cases (especially
    AP invoices with unusual retainage columns, and AR invoices which have a
    completely different multi-line format).

    Status: PASS if diff < 1% of pdf_total OR < $5 absolute.
    """
    out = []
    by_code = {}
    ...
    for cs in code_summaries:
        ...
        # Tolerance: PASS if within 1% of pdf total OR within $5 absolute
        threshold = max(abs(pdf_total) * 0.01, 5.0)
        status = 'PASS' if abs(diff) <= threshold else 'WARN'
        out.append({
...
def main():
    ...
    warned = sum(1 for r in recon if r['status'] == 'WARN')
    print(f'  reconciliation: {passed}/{len(recon)} PASS, {warned} WARN')

    derived = build_derived(line_items, code_summaries, workers, job_totals)

    # ---- Revenue/expense from AUTHORITATIVE PDF totals (not parsed sums) ----
    revenue = abs(job_totals.get('revenues', 0))
    expenses = job_totals.get('expenses', 0)
    net_profit = abs(job_totals.get('net', 0))
    retainage = abs(job_totals.get('retainage', 0))
    net_due = abs(job_totals.get('net_due', 0))

    print(f'  PR hours: {derived["total_labor_hours"]:,.2f}')
    print(f'  Revenue: ${revenue:,.2f}')
    print(f'  Expenses: ${expenses:,.2f}')
    pct = (net_profit/revenue*100) if revenue else 0
    print(f'  Net profit: ${net_profit:,.2f} ({pct:.1f}%)')

    # ---- Build codes dict for downstream build scripts ----
    codes = {}
    for cs in code_summaries:
        codes[cs['code']] = {
            ...
            'regular_hours': cs['regular_hours'],
            'overtime_hours': cs['overtime_hours'],
        }
    ...
    for cs in code_summaries:
        ...
        if cat == 'Revenue':
            continue
        ct = cat_totals.setdefault(cat, {'orig': 0, 'budget': 0, 'actual': 0})
        ct['orig'] += cs['original_budget']
        ct['budget'] += cs['current_budget']
        ct['actual'] += cs['actual_amount']

    # ---- 2020-compatible schema blocks ----
    # Enriched project metadata (matches 2020 project dict)
    project = {
        **PROJECT_META,
        'contract_original': abs(codes.get('999', {}).get('orig', 0)),
        'contract_final': abs(codes.get('999', {}).get('rev', 0)),
    }

    # Totals block (matches 2020 totals dict)
    totals = {
        ...
        'retainage': retainage,
        'net_due': net_due,
        'contract_original': project['contract_original'],
        'contract_final': project['contract_final'],
        'change_orders_net': project['contract_final'] - project['contract_original'],
    }

    # Labor block (matches 2020 labor dict)
    labor = {
        'total_hours': derived['total_labor_hours'],
        'total_workers': derived['total_workers'],
        'straight_time_rate': derived['straight_time_rate'],
        'pr_src_cost_per_hr': derived['pr_src_cost_per_hr'],
        'fully_loaded_wage': derived['fully_loaded_wage'],
        'burden_multiplier': derived['burden_multiplier'],
        'wage_percentiles': derived['nominal_wage_percentiles'],
    }

    data = {
        'schema': 'CORTEX_V2.2',
        'schema_note': 'STUB: No JDR yet for 2118. Placeholder schema matches 2020 gold standard + 2049 v2.2 extensions. Will be populated when Sage publishes a Job Detail Report.',
        # 2020-compatible top-level blocks
        'project': project,
        'totals': totals,
        'labor': labor,
        # 2049/v2.2 extensions
        'report_record': {
            'job_number': '2118',
            'job_name': 'Exxel, Edmonds Behar',
            'report_date': '2026-04-03',
            'line_items_count': len(line_items),
            'cost_code_count': len(code_summaries),
            'job_totals_revenue': -revenue,
            'job_totals_expenses': expenses,
            'job_totals_net': -net_profit,
            'job_totals_net_due': -net_due,
            'job_totals_retainage': -retainage,
            'job_totals_by_source': job_totals.get('by_source', {}),
        },
        'codes': codes,
        'cost_codes': code_summaries,       # list format (2020 compat)
        'cost_code_summaries': code_summaries,  # alias (2049 compat)
        'line_items': line_items,
        'workers': workers,                 # 2020 compat key
        'worker_wages': workers,            # 2049 compat alias
