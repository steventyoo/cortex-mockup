#!/usr/bin/env python3
"""
build_2104.py — Build the 17-tab JCR Cortex v2 workbook for Job 2104
(BMDC, Big 1410) — live/active project.

Input:   2104_data.json  (from parse_2104.py)
Output:  OWP_2104_JCR_Cortex_v2.xlsx
...
DATA_FILE = _HERE / '2104_data.json'
OUT_FILE = _HERE / 'OWP_2104_JCR_Cortex_v2.xlsx'
...
def tab_job_info(wb, data, meta):
    ...
    rows = [
        ...
        ("RI Foreman", meta['ri_foreman']),
        ("Status", "ACTIVE · in-progress · in trim"),
        ("JDR report date", meta['report_date']),
        ("Location", meta['location']),
        ("GC Subcontract #", meta['gc_subcontract_number']),
    ]
    r = 3
    for lbl, val in rows:
...
def generate_predictive_signals(data):
    ...
    # Retention
    ret = abs(META_2104.get('retention', 0))
    billed = abs(META_2104.get('billed', 0))
...
META_2104 = {
    'job_id': '2104',
    'name': 'Big 1410 Apartments',
    'short_name': 'Big 1410',
    'gc': 'BMDC Services, LLC',
    'owner': 'BMDC Services, LLC',
    'units': 24,
    'total_fixtures': 200,
    'insurance': 'Standard (COI)',
    'job_start': 'Jun 2024',
    'expected_completion': 'TBD (active — in trim)',
    'architect': 'N/A',
    'mep': 'N/A',
    'ri_foreman': 'Tommy/Ben',
    'trim_foreman': 'Tommy/Ben',
    'gc_pm': None,   # TODO: capture from Job Ticket (BMDC Services contact)
    'gc_pe': None,   # TODO: capture from Job Ticket
    'gc_project_number': None,  # TODO: capture from BMDC contract docs
    ...
    'base_bid': 1143800,        # matches contract_original — no separate bid sheet; update if OWP bid delta captured
    'bid_date': None,           # TODO: capture from BMDC bid invitation (pre-Jun 2024)
    'previous_plumber': None,   # TODO: N/A or capture if OWP took over from prior sub
    'executed_co_count': 4,
    'cor_count': 5,
    'rfi_count': 7,
    'asi_count': 0,
    ...
    'permit_count': 8,
    'pay_app_count': 18,
    'invoice_count': 226,       # AP line items in 2104_data.json
    'vendor_quote_count': 18,   # files in Vendor Quotes folder
    # COs from SOV (authoritative) + Budget Transfer cross-reference
    # CO#1 = CR1053 Overflow Drains (Budget Transfer CO#1)
    # CO#1.2 = CR1109 Add Irrigation POC (Budget Transfer CO#1.2)
    # CO#4 = CR1080 Core Drill for Other Subs (Budget Transfer CO#4)
    # CO#6 = CR1137 Add Booster Pump (Budget Transfer CO#6)
    # COR#01 Add Condensate Piping — NOT executed
    'cos': [
        {'co': 'CO#1', 'desc': 'Storm Overflow Drains (CR1053)', 'amount': 7164},
        {'co': 'CO#1.2', 'desc': 'Add Irrigation POC (CR1109)', 'amount': 2573},
        {'co': 'CO#4', 'desc': 'Core Drill for Other Subs (CR1080)', 'amount': 3812},
        {'co': 'CO#6', 'desc': 'Add Booster Pump (CR1137)', 'amount': 39200},
    ],
    'pay_apps': [
        # 18 pay apps from Jul 2024 to Mar 2026 — full history in GDrive Billing/SOV
    ],
    'change_log': [
        # COR log from GDrive
        ('COR#01', 'COR', 'Add Condensate Piping', 0, 'NOT EXECUTED'),
        ('COR#02', 'COR', 'Core Drilling', 3812, 'EXECUTED as CO#4'),
        ('COR#03', 'COR', 'Adding Overflows', 7164, 'EXECUTED as CO#1'),
        ('COR#04', 'COR', 'Added Irrigation', 2573, 'EXECUTED as CO#1.2'),
        ('COR#05', 'COR', 'Add Booster Pump', 39200, 'EXECUTED as CO#6'),
    ],
    'predictive_signals': [],
}
...
if __name__ == "__main__":
    data = load_data()
    META_2104['predictive_signals'] = generate_predictive_signals(data)
    out = build_workbook(dict(META_2104))
    print(f"Built: {out}")
