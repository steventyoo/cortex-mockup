#!/usr/bin/env python3
"""
notionize_2104.py — Build the Notion-styled 2-tab summary workbook for
Job 2104 (BMDC, Big 1410) — live project.

Input:   2104_data.json  (from parse_2104.py)
Output:  OWP_2104_JCR_Summary_Notion_v2.xlsx
"""
import json
...
OUT_FILE = _HERE / 'OWP_2104_JCR_Summary_Notion_v2.xlsx'
...
def build_summary_tab(wb, data, meta):
    ...
    ws["A2"] = f"{meta['gc']} · {meta['units']} units · ACTIVE · {meta['pct_complete']:.1%} billed · COI"
...
def generate_predictive_signals(data):
    ...
    signals.append(('Retention', '✓ OK', f'${59686.70:,.0f} held · 5% standard'))
...
if __name__ == "__main__":
    ...
    out = build_notion_workbook(dict(META_2104))
    print(f"Built: {out}")
