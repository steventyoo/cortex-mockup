#!/usr/bin/env python3
"""
build_2043.py — Build OWP_2043_JCR_Cortex_v2.xlsx (17 tabs)

Greenfield 17-tab JCR for Job 2043 Compass Harbor Aegis Mercer Island.
...
Reads /sessions/gracious-relaxed-pascal/2043_data.json from parse_2043.py.
...
DATA_JSON = Path('/sessions/gracious-relaxed-pascal/2043_data.json')
ENRICHED_META_JSON = Path('/sessions/gracious-relaxed-pascal/2043_enriched_meta.json')
OUT_DIR = Path('/sessions/gracious-relaxed-pascal/mnt/cortex-mockup/owp/owp-2043/cortex output files')
OUT_DIR.mkdir(parents=True, exist_ok=True)
OUT_FILE = OUT_DIR / 'OWP_2043_JCR_Cortex_v2.xlsx'
...
def build_overview(wb, d):
    ...
    title(ws, 'OWP JOB 2043 — COMPASS HARBOR AEGIS MERCER ISLAND')
...
def build_job_info(wb, d):
    ...
    title(ws, 'JOB INFORMATION — 2043')
    caption(ws, 'Project identity + metadata enriched from GDrive close-out package (Aegis Mercer Island project info PDF, signed subcontract, permits, scope).')
    ws.append([])
    hdr_row(ws, 3, ['Field', 'Value', 'Source / Notes'])
    rows = [
        ('job_number',         rr['job_number'],                                          'JDR cover page'),
        ('job_name',           em.get('job_name') or rr['job_name'],                      'Aegis Mercer Island project info PDF'),
        ('job_name_long',      em.get('job_name_long', rr['job_name']),                   'Aegis Mercer Island project info PDF'),
        ('general_contractor', em.get('general_contractor'),                              'Signed subcontract SC-17-04-022-100-000 — Shelter Holdings / ASC Mercer Island, LLC'),
        ('owner',              em.get('owner'),                                           'Signed subcontract §A.a — ASC Mercer Island, LLC'),
        ('architect',          em.get('architect'),                                       'Contract counterparty: ASC Mercer Island, LLC'),
        ('location',           em.get('location'),                                        'Signed subcontract + permit records'),
        ('project_type',       em.get('project_type'),                                    'Scope doc + submittal TOC (4-story mixed-use + 2 parking levels)'),
        ('unit_count',         em.get('units'),                                           'Plan drawings: 7-story, 72 units (studios, open 1-BR, 1-BR, 2-BR, 4 Type A ADA)'),
        ...
        ('project_start_date', em.get('start_date'),                                      'JDR first posting 9/8/17 → contract 7/21/17 → construction start Sep 2017'),
        ('project_end_date',   em.get('end_date'),                                        'Final lien release 12/12/2019'),
        ('duration_months',    em.get('duration_months'),                                 'start → end'),
        ('contract_signed',    em.get('contract_signed_date'),                            'Signed subcontract — Mike Donelson 3/9/18'),
        ('permit_jurisdiction', em.get('permit_jurisdiction'),                            'City of Mercer Island permit (Phase I + Phase II plumbing)'),
        ('permits_issued',     '; '.join(em.get('permits_issued', [])),                   'Permits folder: 3 permit types × renewals/extensions'),
        ('status',             'CLOSED',                                                  'Inferred from JDR filter: "Job 2043 only, Open, Closed"'),
...
def build_change_orders(wb, d):
    ...
    title(ws, 'CHANGE ORDER LOG — 2043')
...
def build_project_team(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'PROJECT TEAM — 2043')
    caption(ws, 'Primary contacts extracted from Aegis Mercer Island project start-up PDF + signed subcontract.')
    ws.append([])
...
def build_document_pipeline(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'DOCUMENT PIPELINE — 2043')
    caption(ws, 'Quantitative document counts from the Aegis Mercer Island GDrive close-out package — used by the Cortex dashboard for CO/RFI/Submittal density metrics.')
    ws.append([])
    ...
    rows = [
        ...
        ('Lien releases',                em.get('lien_release_count'),      'Billing/ (LUNA Ave Lien Release #1–19 minus gaps)'),
        ('Permits issued',               len(em.get('permits_issued', [])), 'Permits/'),
        ('Backcharges',                  em.get('backcharge_count'),        'Change Orders/COR_s/ (COR #11 Damaged Scissor Lift)'),
    ]
...
def build_contract(wb, d):
    ...
    title(ws, 'CONTRACT SUMMARY — 2043')
...
def build_cost_code_detail(wb, d):
    ...
    title(ws, 'COST CODE DETAIL — 2043')
...
def build_wage_tiers(wb, d):
    ...
    workers = d['worker_wages']
    df = d['derived_fields']
    title(ws, 'WAGE TIERS & PERCENTILES — 2043')
...
def build_productivity(wb, d):
    ...
    title(ws, 'PRODUCTIVITY METRICS — 2043')
    ...
    rows = [
        ...
        ('Units (residential)',      units,                                            'Aegis Mercer Island scope doc: 81 Type B + 10 Type A + 16 memory care + 2 special'),
...
def build_benchmarks(wb, d):
    ...
    df = d['derived_fields']
    title(ws, 'BENCHMARKS & DERIVED FIELDS — 2043')
...
def build_predictive(wb, d):
    ...
    df = d['derived_fields']
    cs = d['cost_code_summaries']
    title(ws, 'PREDICTIVE SIGNALS — 2043')
...
def build_reconciliation_log(wb, d):
    ...
    title(ws, 'RECONCILIATION LOG — JOB 2043')
    caption(ws, 'Worked example of the Reconciliation Protocol running against Job 2043 (JCR-only, no external cross-checks). Mirrors Sam Test-Labels v4 Reconciliation Log tab.')
    ws.append([])
    ...
    rows = [
        ...
        ('RC-06', 'scope_metrics / scope_benchmarks', 'pending', '—',
         ...
         'Awaiting plumbing scope-metric declarations for Job 2043 (fixture count, DFU total, linear feet of main). Same status as Job 2020.'),
