#!/usr/bin/env python3
"""
parse_2050.py — Parse Job 2050 JDR PDF → 2050_data.json

Structured parse of the 94-page Sage Timberline Job Detail Report for
Job 2050 (GRE Metro Line Flats). Mirrors Sam's v4 Test-Labels
schema field-for-field so downstream Cortex builders can consume the same shape.

Customer code: 2050SH · Client: Boren and Spruce, LLC · GC: GRE.
"""
import json
import re
...
PDF = Path('/sessions/gracious-relaxed-pascal/2050_Job_Detail_Report.pdf')
OUT = Path('/sessions/gracious-relaxed-pascal/2050_data.json')
...
PAGE_HDR_MARKERS = (
    'OWP, LLC Page:', 'Job Detail Report', 'for Job "2050" only', 'Job Number:2050',
    'Job Name: Compass 2100 E Madison', 'Legal:',
...
PR2_RE = re.compile(
    r'^(\d{2}/\d{2}/\d{2})\s+(Regular|Overtime|Doubletime|Other|Vacation|Holiday|Sick):\s+([\d,.\-]+)\s+hours\s+([\d,.\-]+)\s+Ck\s*#:\s*(\S+)$'
)
...
def main():
    ...
    data = {
        'schema': 'CORTEX_V2.2',
        'schema_note': 'Parsed direct from 94-page 2050 JDR PDF. Mirrors Sam v4 test-labels structure.',
