#!/usr/bin/env python3
"""
build_2050_testlabels.py — Build OWP_2050_JCR_Test_Labels.xlsx
...
Consumes /sessions/gracious-relaxed-pascal/2050_data.json produced by parse_2050.py.
"""
import json
...
OUT_FILE = OUT_DIR / 'OWP_2050_JCR_Test_Labels.xlsx'
...
def build_report_record(wb, d):
    ...
    write_title(ws, 'JCR TEST LABELS — JOB 2050',
    ...
    scope_metrics_val = ('50 residential units (47 standard + 3 Type A ADA) + 3 retail · ~395 fixtures (scope estimate) · mid-rise + P1 garage · 18-month duration · 8 executed COs · 19 CORs · 10 permits · 224 invoices') if em else 'pending — trade-declared array of {label, count, description}'
    rows = [
...
def build_reconciliation_log(wb, d):
    ...
    write_title(ws, 'RECONCILIATION LOG — Job 2050',
                'Worked example of the Reconciliation Protocol running against Job 2050 (JCR-only — no external cross-checks). Each row shows the JCR-derived value, the cross-check value, and the observed status. Protocol contract lives in JCR Schema v4 → Reconciliation Protocol tab.')
    ws.append([])
    ...
    rows = [
        ...
        ('RC-06', 'scope_metrics[] / scope_benchmarks', 'pending', '—',
         ...
         'Awaiting plumbing scope-metric declarations for Job 2050 (e.g., fixture count, DFU total, linear feet of main). Labeler should not emit scope_benchmarks until scope_metrics is populated.'),
    ...
    ws.append([])
    ws.append(['How to use: when a labeler produces a new set of derived values for Job 2050, update columns C and D in the rows above and set status per the Protocol contract.'])
