#!/usr/bin/env python3
"""
build_2106.py — Build the 17-tab JCR Cortex v2 workbook for Job 2106
(Chinn, 6220 Roosevelt) — pre-construction project.

Input:   2106_data.json  (from parse_2106.py)
Output:  OWP_2106_JCR_Cortex_v2.xlsx

Notes:
- Pre-construction projects use estimated/engineering basis (no active billed work).
...
DATA_FILE = _HERE / '2106_data.json'
OUT_FILE = _HERE / 'OWP_2106_JCR_Cortex_v2.xlsx'
...
def tab_overview(wb, data, meta):
    ...
    ws["A1"].font = F_H1
    ws["A2"] = f"{meta['gc']} · {meta['units']} units · Pre-construction"
    ws["A2"].font = F_NOTE
...
def tab_job_info(wb, data, meta):
    ...
    rows = [
        ...
        ("Status", "PRE-CONSTRUCTION · estimating/engineering"), ("JDR report date", meta['report_date']),
    ]
    r = 3
    for lbl, val in rows:
...
def tab_predictive(wb, data, meta):
    ...
    ws["A1"] = "Predictive Signals (pre-construction project)"; ws["A1"].font = F_H1
...
def tab_change_log(wb, data, meta):
    ...
    ws.cell(row=4, column=2, value="v1.0 · pre-construction").font = F_BODY
    ws.cell(row=4, column=3, value=f"Initial build for #{meta['job_id']} pre-construction project. 17-tab JCR schema on revised budget basis.").font = F_BODY
...
def generate_predictive_signals(data):
    ...
    # Retention
    ret = abs(META_2106.get('retention', 0))
    billed = abs(META_2106.get('billed', 0))
...
if __name__ == "__main__":
    data = load_data()
    META_2106['predictive_signals'] = generate_predictive_signals(data)
    out = build_workbook(dict(META_2106))
    print(f"Built: {out}")
