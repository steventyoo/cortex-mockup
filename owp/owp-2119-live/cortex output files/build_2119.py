#!/usr/bin/env python3
"""
build_2119.py — Build the 17-tab JCR Cortex v2 workbook for Job 2119
(Holland, Ballard Blossom) — live/active project (pre-construction / DD phase).

Input:   2119_data.json  (from parse_2119.py)
Output:  OWP_2119_JCR_Cortex_v2.xlsx
...
DATA_FILE = _HERE / '2119_data.json'
OUT_FILE = _HERE / 'OWP_2119_JCR_Cortex_v2.xlsx'
...
META_2119 = {
    'job_id': '2119',
    'name': 'The Frank (508 Fremont)',
    'short_name': 'The Frank',
    ...
    'ri_foreman': None,
    'trim_foreman': None,
    'gc_pm': 'Michael Fisher',              # Exxel PM (Bellingham)
    'gc_pe': None,
    'owp_vp': 'Richard Donelson',
    'previous_plumber': None,
    'invoice_count': None,
    'vendor_quote_count': None,
    # Mockup proposal (Nov 13, 2024)
    'mockup_proposal_total': 4700,
    'mockup_proposal_date': '2024-11-13',
    'bid_set_status': 'Sent by Exxel to OWP — pending OWP full proposal',
    'status_note': 'Pre-bid / mockup-only phase. No JDR. No full construction bid. Only $4,700 mockup-install proposal Nov 13 2024. Next milestone: OWP full plumbing proposal to Exxel (Michael Fisher).',
    'cos': [
...
def generate_predictive_signals(data):
    ...
    # Retention
    ret = abs(META_2119.get('retention', 0))
    billed = abs(META_2119.get('billed', 0))
...
if __name__ == "__main__":
    data = load_data()
    META_2119['predictive_signals'] = generate_predictive_signals(data)
    out = build_workbook(dict(META_2119))
    print(f"Built: {out}")
