#!/usr/bin/env python3
"""
build_2048.py — Build OWP_2048_JCR_Cortex_v2.xlsx (17 tabs)

Greenfield 17-tab JCR for Job 2048 GRE Ballinger.
...
Reads /sessions/gracious-relaxed-pascal/2048_data.json from parse_2048.py.
...
OUT_FILE = OUT_DIR / 'OWP_2048_JCR_Cortex_v2.xlsx'
...
def build_overview(wb, d):
    ...
    title(ws, 'OWP JOB 2048 — GRE BALLINGER')
...
def build_job_info(wb, d):
    ...
    title(ws, 'JOB INFORMATION — 2048')
    caption(ws, 'Project identity + metadata enriched from GDrive close-out package (GRE Ballinger Apartments project info PDF, signed subcontract, permits, scope).')
    ws.append([])
    hdr_row(ws, 3, ['Field', 'Value', 'Source / Notes'])
    rows = [
        ('job_number',         rr['job_number'],                                          'JDR cover page'),
        ('job_name',           em.get('job_name') or rr['job_name'],                      'GRE Ballinger Apartments project info PDF'),
        ('job_name_long',      em.get('job_name_long', rr['job_name']),                   'GRE Ballinger Apartments project info PDF'),
        ('general_contractor', em.get('general_contractor'),                              'Signed subcontract 7921 — Shelter Holdings / GRE 205th LLC'),
        ('owner',              em.get('owner'),                                           'Signed subcontract §A.a — GRE 205th LLC'),
        ('architect',          em.get('architect'),                                       'Contract counterparty: GRE 205th LLC'),
        ...
        ('status',             'CLOSED',                                                  'Inferred from JDR filter: "Job 2048 only, Open, Closed"'),
...
def build_change_orders(wb, d):
    ...
    title(ws, 'CHANGE ORDER LOG — 2048')
...
def build_project_team(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'PROJECT TEAM — 2048')
    caption(ws, 'Primary contacts extracted from GRE Ballinger Apartments project start-up PDF + signed subcontract.')
    ws.append([])
...
def build_document_pipeline(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'DOCUMENT PIPELINE — 2048')
    caption(ws, 'Quantitative document counts from the GRE Ballinger Apartments GDrive close-out package — used by the Cortex dashboard for CO/RFI/Submittal density metrics.')
    ws.append([])
    ...
    rows = [
        ...
        ('Backcharges',                  em.get('backcharge_count'),        'Change Orders/COR_s/ (no backcharges logged for GRE Ballinger)'),
    ]
...
def build_contract(wb, d):
    ...
    title(ws, 'CONTRACT SUMMARY — 2048')
...
def build_cost_code_detail(wb, d):
    ...
    title(ws, 'COST CODE DETAIL — 2048')
...
def build_wage_tiers(wb, d):
    ...
    workers = d['worker_wages']
    df = d['derived_fields']
    title(ws, 'WAGE TIERS & PERCENTILES — 2048')
...
def build_productivity(wb, d):
    ...
    title(ws, 'PRODUCTIVITY METRICS — 2048')
    ...
    rows = [
        ...
        ('Units (residential)',      units,                                            'GRE Ballinger Apartments scope doc: 68 Type B + 4 Type A ADA'),
...
def build_benchmarks(wb, d):
    ...
    df = d['derived_fields']
    title(ws, 'BENCHMARKS & DERIVED FIELDS — 2048')
...
def build_predictive(wb, d):
    ...
    df = d['derived_fields']
    cs = d['cost_code_summaries']
    title(ws, 'PREDICTIVE SIGNALS — 2048')
...
def build_reconciliation_log(wb, d):
    ...
    title(ws, 'RECONCILIATION LOG — JOB 2048')
    caption(ws, 'Worked example of the Reconciliation Protocol running against Job 2048 (JCR-only, no external cross-checks). Mirrors Sam Test-Labels v4 Reconciliation Log tab.')
    ws.append([])
    ...
    rows = [
        ...
        ('RC-06', 'scope_metrics / scope_benchmarks', 'pending', '—',
         ...
         'Awaiting plumbing scope-metric declarations for Job 2048 (fixture count, DFU total, linear feet of main). Same status as Job 2020.'),
