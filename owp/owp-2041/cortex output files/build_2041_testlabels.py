#!/usr/bin/env python3
"""
build_2041_testlabels.py — Build OWP_2041_JCR_Test_Labels.xlsx
...
Consumes /sessions/gracious-relaxed-pascal/2041_data.json produced by parse_2041.py.
"""
import json
...
OUT_FILE = OUT_DIR / 'OWP_2041_JCR_Test_Labels.xlsx'
...
def build_report_record(wb, d):
    ...
    write_title(ws, 'JCR TEST LABELS — JOB 2041',
    ...
    scope_metrics_val = ('108 residential units (102 type A + 6 ADA) · ~620 fixtures (scope estimate) · 4 residential floors + 2 parking levels (P1/P2) · PCC retail ground floor · 24-month duration · 18 executed COs · 74 RFI/ASI responses · 82 submittals · 197 POs') if em else 'pending — trade-declared array of {label, count, description}'
    rows = [
...
def build_reconciliation_log(wb, d):
    ...
    write_title(ws, 'RECONCILIATION LOG — Job 2041',
                'Worked example of the Reconciliation Protocol running against Job 2041 (JCR-only — no external cross-checks). Each row shows the JCR-derived value, the cross-check value, and the observed status. Protocol contract lives in JCR Schema v4 → Reconciliation Protocol tab.')
    ws.append([])
    ...
    rows = [
        ...
        ('RC-06', 'scope_metrics[] / scope_benchmarks', 'pending', '—',
         ...
         'Awaiting plumbing scope-metric declarations for Job 2041 (e.g., fixture count, DFU total, linear feet of main). Labeler should not emit scope_benchmarks until scope_metrics is populated.'),
    ...
    ws.append([])
    ws.append(['How to use: when a labeler produces a new set of derived values for Job 2041, update columns C and D in the rows above and set status per the Protocol contract.'])
