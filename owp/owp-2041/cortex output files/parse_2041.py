#!/usr/bin/env python3
"""
parse_2041.py — Parse Job 2041 JDR PDF → 2041_data.json

Structured parse of the 175-page Sage Timberline Job Detail Report for
Job 2041 (Chinn California Ave Luna). Mirrors Sam's v4 Test-Labels schema
field-for-field so downstream Cortex builders can consume the same shape.

Outputs line_items, cost_code_summaries, worker_wages (PR-grouped),
derived_fields (PR/AP/GL rollups, labor analytics, material analytics,
phase analytics, budget/forecast), reconciliation (per-code parsed-sum
vs PDF-total), and report_record (job totals + by_source).
"""
import json
import re
...
PDF = Path('/sessions/gracious-relaxed-pascal/mnt/cortex-mockup/owp/Gemini Accounting Reports/2041 Job Detail Report.pdf')
OUT = Path('/sessions/gracious-relaxed-pascal/2041_data.json')

PROJECT_META = {
    'job_number': '2041',
    'job_name': 'Chinn California Ave Luna',
    'general_contractor': 'Chinn Construction',
...
PAGE_HDR_MARKERS = (
    'OWP, LLC Page:', 'Job Detail Report', 'for Job "2041" only', 'Job Number:2041',
    'Job Name: Chinn California Ave Luna', 'Legal:',
...
# PR line 1: "PR 545 09/22/17 GE69 Gerard, Jeffrey S"
PR1_RE = re.compile(r'^PR\s+(\S+)\s+(\d{2}/\d{2}/\d{2})\s+(\S+)\s+(.+)$')
# PR line 2: "09/20/17 Regular: 1.00 hours 47.00 Ck #: 17515"
# Also supports Overtime: / Doubletime: / Other:
PR2_RE = re.compile(
    r'^(\d{2}/\d{2}/\d{2})\s+(Regular|Overtime|Doubletime|Other):\s+([\d,.\-]+)\s+hours\s+([\d,.\-]+)\s+Ck\s*#:\s*(\S+)$'
)
# PR burden line 2 (codes 995/998): "09/20/17 Overhead 2 20.84" — no hours, no Ck#
PR2_OH_RE = re.compile(
    r'^(\d{2}/\d{2}/\d{2})\s+Overhead\s+(\d+)\s+([\d,.\-]+)$'
)
# AP line 1: "AP 179 05/09/18 2041RS Rosen Supply Company, Inc. $Cal Luna"
AP1_RE = re.compile(r'^AP\s+(\S+)\s+(\d{2}/\d{2}/\d{2})\s+(\S+)\s+(.+)$')
# AP line 2: "04/23/18 Inv: 4250936-00 349.52" or "05/01/19 Credit: CM8475 -50.00"
#   Net-due + retainage may follow.
AP2_RE = re.compile(
    r'^(\d{2}/\d{2}/\d{2})\s+(Inv|Credit):\s+(\S+)\s+([\d,.\-]+)(?:\s+([\d,.\-]+))?(?:\s+([\d,.\-]+))?$'
)
# GL line 1: "GL 574 05/22/18 CK#5502 Joseph Barnhart"  OR  "GL 132 06/01/18 Adj"
GL1_RE = re.compile(r'^GL\s+(\S+)\s+(\d{2}/\d{2}/\d{2})\s+(\S+)(?:\s+(.+))?$')
# GL line 2: "05/22/18"
GL2_RE = re.compile(r'^(\d{2}/\d{2}/\d{2})$')
# GL line 3: just an amount like "112.18" or "-1,673.59"
GL3_RE = re.compile(r'^([\d,.\-]+)$')
# AR line 1: "AR 750 08/15/19 2041CC Chinn Construction, LLC $Luna-California"
AR1_RE = re.compile(r'^AR\s+(\S+)\s+(\d{2}/\d{2}/\d{2})\s+(\S+)\s+(.+)$')
# AR line 2: "08/20/19 Invoice 037387 2,029.00- 101.45-"   (retainage optional)
AR2_RE = re.compile(
...
def parse():
    ...
    # Pass: clean copy of non-page-header lines
    clean = [L for L in lines if not is_page_header(L)]

    line_items = []
    code_summaries = []  # per-code rollup rows
    job_totals = {}
    current_code = None
    current_desc = None
    # Skip pass: find cost-code headers and assemble
    i = 0
    while i < len(clean):
        L = clean[i].rstrip()
        ...
        # Cost Code Totals (end of current code block)
        ...
        if m:
            ...
            # Look ahead for payroll hours (labor only)
...
def build_reconciliation(line_items, code_summaries):
    """Per-code parsed-sum vs PDF-total reconciliation."""
    out = []
    by_code = {}
...
def build_worker_wages(line_items):
    """Group PR lines by worker → per-worker wage + tier bucket."""
    by_name = {}
    for it in line_items:
        if it['source'] != 'PR':
            continue
    ...
    for w in by_name.values():
        ...
        # Tier (classical plumbing):
        #   < $20/hr → APPRENTICE/HELPER
        #   $20–$36 → JOURNEYMAN
        #   $36+   → LEAD/SUPERVISOR
        #   OT-only (no reg hours) → OT-ONLY
        rate = w['nominal_rate']
        ...
        workers.append(w)
    # Sort by tier (apprentice → journeyman → lead → ot-only) then nominal ascending
...
def build_derived(line_items, code_summaries, workers, job_totals):
    ...
    # Labor hours total (PR only, reg + ot + dt + other)
    total_hours = 0
    reg_hours_sum = 0
    reg_amt_sum = 0
    for it in line_items:
...
def main():
    ...
    data = {
        'schema': 'CORTEX_V2.2',
        'schema_note': 'Parsed direct from 175-page 2041 JDR PDF. Mirrors Sam v4 test-labels structure.',
