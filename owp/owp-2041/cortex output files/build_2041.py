#!/usr/bin/env python3
"""
build_2041.py — Build OWP_2041_JCR_Cortex_v2.xlsx (17 tabs)

Greenfield 17-tab JCR for Job 2041 Chinn California Ave Luna.
...
Reads /sessions/gracious-relaxed-pascal/2041_data.json from parse_2041.py.
...
DATA_JSON = Path('/sessions/gracious-relaxed-pascal/2041_data.json')
ENRICHED_META_JSON = Path('/sessions/gracious-relaxed-pascal/2041_enriched_meta.json')
OUT_DIR = Path('/sessions/gracious-relaxed-pascal/mnt/cortex-mockup/owp/owp-2041/cortex output files')
OUT_DIR.mkdir(parents=True, exist_ok=True)
OUT_FILE = OUT_DIR / 'OWP_2041_JCR_Cortex_v2.xlsx'
...
def build_overview(wb, d):
    ...
    title(ws, 'OWP JOB 2041 — CHINN CALIFORNIA AVE LUNA')
...
def build_job_info(wb, d):
    ...
    title(ws, 'JOB INFORMATION — 2041')
    caption(ws, 'Project identity + metadata enriched from GDrive close-out package (Chinn Luna project info PDF, signed subcontract, permits, scope).')
    ws.append([])
    hdr_row(ws, 3, ['Field', 'Value', 'Source / Notes'])
    rows = [
        ('job_number',         rr['job_number'],                                          'JDR cover page'),
        ('job_name',           em.get('job_name') or rr['job_name'],                      'Chinn Luna project info PDF'),
        ('job_name_long',      em.get('job_name_long', rr['job_name']),                   'Chinn Luna project info PDF'),
        ('general_contractor', em.get('general_contractor'),                              'Signed subcontract cover — Chinn Construction, LLC'),
        ('owner',              em.get('owner'),                                           'Signed subcontract §A.a — 2741 California LLC, Issaquah WA'),
        ('architect',          em.get('architect'),                                       'Signed subcontract §A.b — Hewitt Architects'),
        ('location',           em.get('location'),                                        'Signed subcontract + Chinn start-up PDF'),
        ('project_type',       em.get('project_type'),                                    'Scope doc + submittal TOC (4-story mixed-use + 2 parking levels)'),
        ('unit_count',         em.get('units'),                                           'Scope doc: 102 type A + 6 ADA = 108 residential units'),
        ...
        ('project_start_date', em.get('start_date'),                                      'Chinn start-up PDF: Estimated Start Jan 2 2018'),
        ...
        ('permit_jurisdiction', em.get('permit_jurisdiction'),                            'Chinn start-up PDF: Inspecting Office = City of Seattle'),
        ('permits_issued',     '; '.join(em.get('permits_issued', [])),                   'Permits folder: 3 permit types × renewals/extensions'),
        ('status',             'CLOSED',                                                  'Inferred from JDR filter: "Job 2041 only, Open, Closed"'),
...
def build_change_orders(wb, d):
    ...
    title(ws, 'CHANGE ORDER LOG — 2041')
...
def build_project_team(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'PROJECT TEAM — 2041')
    caption(ws, 'Primary contacts extracted from Chinn Luna project start-up PDF + signed subcontract.')
    ws.append([])
...
def build_document_pipeline(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'DOCUMENT PIPELINE — 2041')
    caption(ws, 'Quantitative document counts from the Chinn Luna GDrive close-out package — used by the Cortex dashboard for CO/RFI/Submittal density metrics.')
    ws.append([])
...
def build_contract(wb, d):
    ...
    title(ws, 'CONTRACT SUMMARY — 2041')
...
def build_cost_code_detail(wb, d):
    ...
    title(ws, 'COST CODE DETAIL — 2041')
...
def build_wage_tiers(wb, d):
    ...
    workers = d['worker_wages']
    df = d['derived_fields']
    title(ws, 'WAGE TIERS & PERCENTILES — 2041')
...
def build_productivity(wb, d):
    ...
    title(ws, 'PRODUCTIVITY METRICS — 2041')
    ...
    rows = [
        ...
        ('Units (residential)',      units,                                            'Chinn Luna scope doc: 102 type A + 6 ADA'),
...
def build_benchmarks(wb, d):
    ...
    df = d['derived_fields']
    title(ws, 'BENCHMARKS & DERIVED FIELDS — 2041')
...
def build_predictive(wb, d):
    ...
    df = d['derived_fields']
    cs = d['cost_code_summaries']
    title(ws, 'PREDICTIVE SIGNALS — 2041')
...
def build_reconciliation_log(wb, d):
    ...
    title(ws, 'RECONCILIATION LOG — JOB 2041')
    caption(ws, 'Worked example of the Reconciliation Protocol running against Job 2041 (JCR-only, no external cross-checks). Mirrors Sam Test-Labels v4 Reconciliation Log tab.')
    ws.append([])
    ...
    rows = [
        ...
        ('RC-06', 'scope_metrics / scope_benchmarks', 'pending', '—',
         ...
         'Awaiting plumbing scope-metric declarations for Job 2041 (fixture count, DFU total, linear feet of main). Same status as Job 2020.'),
