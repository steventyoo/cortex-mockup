#!/usr/bin/env python3
"""
build_2114.py — Build the 17-tab JCR Cortex v2 workbook for Job 2114
(Holland, Ballard Blossom) — live/active project (pre-construction / DD phase).

Input:   2114_data.json  (from parse_2114.py)
Output:  OWP_2114_JCR_Cortex_v2.xlsx
...
DATA_FILE = _HERE / '2114_data.json'
OUT_FILE = _HERE / 'OWP_2114_JCR_Cortex_v2.xlsx'
...
META_2114 = {
    'job_id': '2114',
    'name': 'Ballard Blossom Apartments',
    'short_name': 'Ballard Blossom',
    'gc': 'Holland Construction',
    'owner': None,                         # TBD — Tatemono-Metzler group suspected
    'units': 239,
    'total_fixtures': 1368,                # roughin fixtures per bid (permit fixtures 1,788)
    'insurance': 'OCIP (WRAP) — assumed by Owner',
    'job_start': 'Sep 2025',               # first Sage PR entry (design/takeoff)
    'expected_completion': 'Apr 2028 TCO (22-month field schedule from Jul 2026 foundations)',
    'architect': None,                     # TBD — not captured in Drive search yet
    'mep': 'Robison Engineering',
    'location': '1734 NW Market St, Seattle, WA 98107 (Ballard)',
    'project_type': '8-story + partially sunken basement multifamily + retail',
    'subcontract_number': None,            # TBD — Holland subcontract not filed to Drive yet
    'gc_project_number': None,
    'report_date': 'Apr 3, 2026',
    'billed': 80000.00,
    'retention': 0.00,
    'build_date': '2026-04-24',
    'contract_original': 5670000,
    'contract_final': 5670000,
    'base_bid': 5670000,
    'bid_date': '2025-05-28',               # "reduced price" bid sheet
    'executed_co_count': 0,
    'cor_count': 0,
    'rfi_count': 0,
    'asi_count': 0,
    ...
    'permit_count': 0,
    'pay_app_count': 1,                     # Single design-billing AR
    'ri_foreman': None,                     # TBD — still in design/takeoff phase
    'trim_foreman': None,
    'gc_pm': 'Drew Schemchuck',             # Holland side (per bid sheet note, confirmed 3/6/25)
    'gc_pe': 'Michael Ricci',               # Holland side (per bid sheet note, 5/2/25)
    'owp_vp': 'Richard Donelson',
    'previous_plumber': None,
    'invoice_count': None,                   # pulled from data.json at build time
    'vendor_quote_count': None,
    # Bid-phase budget intelligence (useful for CO/variance reasoning)
    'bid_labor': 1372396,
    ...
    'bid_avg_wage': 43.13,
    'cos': [
        # No executed COs yet — project in pre-construction / design DD
    ],
    'pay_apps': [
        # Single $80,000 design-billing AR (2025-08-08 mockup billing invoice + Oct 2025 / Jan 2026 design invoices)
    ],
    'change_log': [
        # No COR log yet — project pre-CD
    ],
    'predictive_signals': [
        # Auto-generated from JDR BVA analysis below
    ],
}
...
def generate_predictive_signals(data):
    ...
    # Retention
    ret = abs(META_2114.get('retention', 0))
    billed = abs(META_2114.get('billed', 0))
...
if __name__ == "__main__":
    data = load_data()
    META_2114['predictive_signals'] = generate_predictive_signals(data)
    out = build_workbook(dict(META_2114))
    print(f"Built: {out}")
