#!/usr/bin/env python3
"""
build_2046.py — Build OWP_2046_JCR_Cortex_v2.xlsx (17 tabs)

Greenfield 17-tab JCR for Job 2046 Marpac 4218 Roosevelt.
...
Reads /sessions/gracious-relaxed-pascal/2046_data.json from parse_2046.py.
...
DATA_JSON = Path('/sessions/gracious-relaxed-pascal/2046_data.json')
ENRICHED_META_JSON = Path('/sessions/gracious-relaxed-pascal/2046_enriched_meta.json')
OUT_DIR = Path('/sessions/gracious-relaxed-pascal/mnt/cortex-mockup/owp/owp-2046/cortex output files')
OUT_DIR.mkdir(parents=True, exist_ok=True)
OUT_FILE = OUT_DIR / 'OWP_2046_JCR_Cortex_v2.xlsx'
...
def build_overview(wb, d):
    ...
    title(ws, 'OWP JOB 2046 — MARPAC 4218 ROOSEVELT')
...
def build_job_info(wb, d):
    ...
    title(ws, 'JOB INFORMATION — 2046')
    caption(ws, 'Project identity + metadata enriched from GDrive close-out package (4218 Roosevelt Apartments project info PDF, signed subcontract, permits, scope).')
    ws.append([])
    hdr_row(ws, 3, ['Field', 'Value', 'Source / Notes'])
    rows = [
        ('job_number',         rr['job_number'],                                          'JDR cover page'),
        ('job_name',           em.get('job_name') or rr['job_name'],                      '4218 Roosevelt Apartments project info PDF'),
        ('job_name_long',      em.get('job_name_long', rr['job_name']),                   '4218 Roosevelt Apartments project info PDF'),
        ('general_contractor', em.get('general_contractor'),                              'Signed subcontract 17-10 — Shelter Holdings / G&K Investment LP'),
        ('owner',              em.get('owner'),                                           'Signed subcontract §A.a — G&K Investment LP'),
        ('architect',          em.get('architect'),                                       'Contract counterparty: G&K Investment LP'),
        ...
        ('unit_count',         em.get('units'),                                           'Plan drawings: 7-story, 120 units (studios, open 1-BR, 1-BR, 2-BR, 4 Type A ADA)'),
        ...
        ('status',             'CLOSED',                                                  'Inferred from JDR filter: "Job 2046 only, Open, Closed"'),
...
def build_change_orders(wb, d):
    ...
    title(ws, 'CHANGE ORDER LOG — 2046')
...
def build_project_team(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'PROJECT TEAM — 2046')
    caption(ws, 'Primary contacts extracted from 4218 Roosevelt Apartments project start-up PDF + signed subcontract.')
    ws.append([])
...
def build_document_pipeline(wb, d):
    ...
    em = d.get('enriched_meta', {})
    title(ws, 'DOCUMENT PIPELINE — 2046')
    caption(ws, 'Quantitative document counts from the 4218 Roosevelt Apartments GDrive close-out package — used by the Cortex dashboard for CO/RFI/Submittal density metrics.')
    ws.append([])
    ...
    rows = [
        ...
        ('Backcharges',                  em.get('backcharge_count'),        'Change Orders/COR_s/ (no backcharges logged for 4218 Roosevelt)'),
    ]
...
def build_contract(wb, d):
    ...
    title(ws, 'CONTRACT SUMMARY — 2046')
...
def build_cost_code_detail(wb, d):
    ...
    title(ws, 'COST CODE DETAIL — 2046')
...
def build_wage_tiers(wb, d):
    ...
    workers = d['worker_wages']
    df = d['derived_fields']
    title(ws, 'WAGE TIERS & PERCENTILES — 2046')
...
def build_productivity(wb, d):
    ...
    title(ws, 'PRODUCTIVITY METRICS — 2046')
    ...
    rows = [
        ...
        ('Units (residential)',      units,                                            '4218 Roosevelt Apartments scope doc: 113 Type B + 7 Type A ADA'),
...
def build_benchmarks(wb, d):
    ...
    df = d['derived_fields']
    title(ws, 'BENCHMARKS & DERIVED FIELDS — 2046')
...
def build_predictive(wb, d):
    ...
    df = d['derived_fields']
    cs = d['cost_code_summaries']
    title(ws, 'PREDICTIVE SIGNALS — 2046')
...
def build_reconciliation_log(wb, d):
    ...
    title(ws, 'RECONCILIATION LOG — JOB 2046')
    caption(ws, 'Worked example of the Reconciliation Protocol running against Job 2046 (JCR-only, no external cross-checks). Mirrors Sam Test-Labels v4 Reconciliation Log tab.')
    ws.append([])
    ...
    rows = [
        ...
        ('RC-06', 'scope_metrics / scope_benchmarks', 'pending', '—',
         ...
         'Awaiting plumbing scope-metric declarations for Job 2046 (fixture count, DFU total, linear feet of main). Same status as Job 2020.'),
