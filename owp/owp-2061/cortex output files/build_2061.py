#!/usr/bin/env python3
"""
build_2061.py — Build the 17-tab JCR Cortex v2 workbook for Job 2061
(Exxel Pacific, Alta Columbia City) — CLOSED project (Jun 2019 → May 2022).

Input:   2061_data.json  (from parse_2061.py)
Output:  OWP_2061_JCR_Cortex_v2.xlsx

Notes:
- CLOSED projects use revised (final) contract as canonical basis.
- Status shows "CLOSED · 41.4% margin · retention released"
...
DATA_FILE = _HERE / '2061_data.json'
OUT_FILE = _HERE / 'OWP_2061_JCR_Cortex_v2.xlsx'
...
def tab_overview(wb, data, meta):
    ...
    ws["A1"].font = F_H1
    ws["A2"] = f"{meta['gc']} · {meta['units']} units · CLOSED"
    ws["A2"].font = F_NOTE
...
def tab_job_info(wb, data, meta):
    ...
    rows = [
        ...
        ("Status", "CLOSED"), ("JDR report date", meta['report_date']),
    ]
    r = 3
    for lbl, val in rows:
...
def tab_predictive(wb, data, meta):
    ...
    ws["A1"] = "Predictive Signals (closed project)"; ws["A1"].font = F_H1
...
def tab_change_log(wb, data, meta):
    ...
    ws.cell(row=4, column=2, value="v1.0 · closed").font = F_BODY
    ws.cell(row=4, column=3, value=f"Initial build for #{meta['job_id']} closed project. 17-tab JCR schema on revised budget basis.").font = F_BODY
...
META_2061 = {
    ...
    'gc': "Exxel Pacific",
    'units': 243,
    'total_fixtures': None,  # Fixture types confirmed (WC1, L1, TS1/TS2, SS1/2/3, S1/S2, AW, DW, etc.) — total count not aggregated yet
    'insurance': "OCIP (Wrap-up) — Exxel-administered",
    'job_start': "2019-06",
    'expected_completion': "2022-05",
    'architect': "Johnston Architects, PLLC",
    'mep': "Franklin Engineering",
    'location': "3717 South Alaska Street, Seattle, WA 98118 (Columbia City)",
    'project_type': "243-unit mixed-use multifamily + 6 retail spaces",
    'foreman': "TBD (per JDR — to be enriched from Job Tickets)",
    'status': "CLOSED · 41.4% margin · retention released 2022-06-23",
    ...
    'executed_co_count': 13,    # 13 unique SCO numbers (CO #001 through SCO 17)
    'cor_count': 14,             # ~14 unique CORs (97 files include revisions)
    'rfi_count': 39,             # 39 distinct RFIs in folder
    'asi_count': 26,             # 26 distinct ASIs (highest = ASI 034)
    'submittal_count': 117,      # 117 files in Submittals folder
    'total_pos': 290,            # 36 placed + 149 scheduled + 105 completed (overlap exists; canonical = 290 unique POs)
    'permit_count': 209,         # 209 files in Permits folder
    'pay_app_count': 27,         # 27 AR transactions (per dashboard payApps)
    'gc_pm': "Brian Christensen (Exxel)",
    'gc_sup': "TBD",
    'gc_pe': "TBD",
    'ri_foreman': "TBD",
    'owp_signatory': "Richard Donelson",
    'project_duration_months': 35,  # Jun 2019 → May 2022 ≈ 35 months
    'cos': [],
    'pay_apps': [],
    'change_log': [],
    'predictive_signals': [],
...
def generate_predictive_signals(data):
    ...
    # Retention
    ret = abs(META_2061.get('retention', 0))
    billed = abs(META_2061.get('billed', 0))
    ret_pct = ret / billed if billed else 0
    signals.append((
        'Retention aging',
        '✓ OK' if ret_pct < 0.06 else '⚠ WATCH',
        f'${ret:,.0f} held · {ret_pct:.1%} of billed · zero (closed)'
    ))
...
if __name__ == "__main__":
    data = load_data()
    META_2061['predictive_signals'] = generate_predictive_signals(data)
    out = build_workbook(dict(META_2061))
    print(f"Built: {out}")
