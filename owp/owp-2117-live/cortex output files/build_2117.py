#!/usr/bin/env python3
"""
build_2117.py — Build the 17-tab JCR Cortex v2 workbook for Job 2117
(Holland, Ballard Blossom) — live/active project (pre-construction / DD phase).

Input:   2117_data.json  (from parse_2117.py)
Output:  OWP_2117_JCR_Cortex_v2.xlsx
...
DATA_FILE = _HERE / '2117_data.json'
OUT_FILE = _HERE / 'OWP_2117_JCR_Cortex_v2.xlsx'
...
META_2117 = {
    'job_id': '2117',
    'name': 'Lachlan Apartments',
    'short_name': 'Lachlan',
    'gc': 'Ravenna Partners',
    'owner': 'Ravenna Partners',
    'units': 104,                           # 98 Type B + 6 Type A = 104 toilets / units
    'total_fixtures': None,
    'insurance': 'Standard (COI)',
    'job_start': 'Feb 2026',
    'expected_completion': 'TBD — schedule not published',
    'architect': None,
    'mep': 'Franklin Engineering',          # FEProp Jan 22 2026 suggests Franklin
    ...
    'contract_original': None,              # Contract value not yet captured
    'contract_final': None,
    'base_bid': None,
    ...
    'water_heating_system': 'Gas (3x 100gal, 199mbh vertical)',  # Not heat pump
...
def generate_predictive_signals(data):
    ...
    # Retention
    ret = abs(META_2117.get('retention', 0))
    billed = abs(META_2117.get('billed', 0))
...
if __name__ == "__main__":
    data = load_data()
    META_2117['predictive_signals'] = generate_predictive_signals(data)
    out = build_workbook(dict(META_2117))
    print(f"Built: {out}")
