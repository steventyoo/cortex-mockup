#!/usr/bin/env python3
"""
parse_2116.py — Parse Job 2116 JDR PDF → 2116_data.json

Structured parse of the 9-page Sage Timberline Job Detail Report for
Job 2116 (SRM, The V Apartments) — 194-unit Seattle apartment with GMP
contract executed Feb 24, 2026. Pre-construction / early-mobilization
phase as of the Apr 3, 2026 JDR snapshot (takeoff labor only, COR#2
in flight). Mirrors Cortex v2.2 schema so downstream build/notionize
scripts can consume the same shape.

Customer code: 2116SR · Client: SRM Construction, Inc. · GC: SRM
(Erik Benzel, PM · Spokane office).
"""
import json
import re
...
# owp-2116 live/cortex output files/ → owp/ → Gemini Accounting Reports/
PDF = _HERE.parent.parent / 'Gemini Accounting Reports' / '2116 Job Detail Report.pdf'
OUT = _HERE / '2116_data.json'

PROJECT_META = {
    'job_number': '2116',
    'job_name': 'The V Apartments',
    'general_contractor': 'SRM Construction, Inc.',
    'owner': 'NYL (per executed GMP contract)',
    'location': 'Seattle, WA',
    'project_type': '7-story multifamily (194 residential units)',
    'units': 194,
    'total_fixtures': None,
    'architect': None,
    'mep_engineer': 'Emerald City Engineers (ECE)',
    'report_date': '2026-04-03',
    'contract_original': 3258000,
    'total_cos_net': 0,
    'contract_final': 3258000,
    'executed_co_count': 0,
    'cor_count': 1,                       # COR#2 Plumbing Changes (Mar 25, 2026) — in flight
    'cancelled_cor_count': 0,
    'rfi_count': 0,
    'asi_count': 0,
    'submittal_count': 2,                 # Underground roughin (Jan 20 2026) + Unit Fixture Submittals (Mar 19 2026)
    'total_pos': 0,
    'permit_count': 0,
    'pay_app_count': 0,
    ...
    'gmp_contract_executed': '2026-02-24',
    'fixture_schedule_summary': '194 Toto MS436124CEMFGN toilets · 103 tub/shower stalls · 60 Mincey shower pans · 166 kitchen sinks · 26 Sanco2 heat pump units · 3 storage tanks',
    'value_engineering_status': 'Multiple VE options pending (Gas WH, PVI Gas, Lochinvar boilers)',
}
...
PAGE_HDR_MARKERS = (
    'OWP, LLC', 'Job Detail Report', 'for Job "2116"', 'Job Number: 2116',
    'Job Name: SRM, The V Apts', 'Legal:',
...
def main():
    ...
    data = {
        'schema': 'CORTEX_V2.2',
        'schema_note': 'Parsed direct from 9-page 2116 JDR PDF. Pre-construction / early-mobilization phase (GMP executed Feb 24 2026, COR#2 in flight). Schema matches 2020 gold standard + 2049 v2.2 extensions.',
