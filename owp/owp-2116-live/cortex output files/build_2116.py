#!/usr/bin/env python3
"""
build_2116.py — Build the 17-tab JCR Cortex v2 workbook for Job 2116
(Holland, Ballard Blossom) — live/active project (pre-construction / DD phase).

Input:   2116_data.json  (from parse_2116.py)
Output:  OWP_2116_JCR_Cortex_v2.xlsx
...
DATA_FILE = _HERE / '2116_data.json'
OUT_FILE = _HERE / 'OWP_2116_JCR_Cortex_v2.xlsx'
...
META_2116 = {
    'job_id': '2116',
    'name': 'The V Apartments',
    'short_name': 'The V',
    'gc': 'SRM Construction, Inc.',
    'owner': 'NYL (per executed GMP contract)',
    'units': 194,
    'total_fixtures': None,
    'insurance': 'Standard (COI)',
    'job_start': 'Apr 2026',                # first Sage PR entry
    'expected_completion': 'TBD — schedule not published',
    'architect': None,
    'mep': 'Emerald City Engineers (ECE)',
    'location': 'Seattle, WA',
    ...
    'base_bid': 3258000,
    'bid_date': '2025-09-13',               # Plumbing proposal to SRM
    'executed_co_count': 0,
    'cor_count': 1,                         # COR#2 Plumbing Changes (Mar 25, 2026) — in flight
    'rfi_count': 0,
    'asi_count': 0,
    'submittal_count': 2,                   # Underground roughin + Unit Fixture
    'total_pos': 0,
    'permit_count': 0,
    'pay_app_count': 0,
    'ri_foreman': None,
    'trim_foreman': None,
    'gc_pm': 'Erik Benzel',                 # SRM PM (Spokane office)
    'gc_pe': None,
    ...
    'fixture_schedule_summary': '194 Toto MS436124CEMFGN toilets · 103 tub/shower stalls · 60 Mincey shower pans · 166 kitchen sinks · 26 Sanco2 heat pump units',
...
def generate_predictive_signals(data):
    ...
    # Retention
    ret = abs(META_2116.get('retention', 0))
    billed = abs(META_2116.get('billed', 0))
...
if __name__ == "__main__":
    data = load_data()
    META_2116['predictive_signals'] = generate_predictive_signals(data)
    out = build_workbook(dict(META_2116))
    print(f"Built: {out}")
