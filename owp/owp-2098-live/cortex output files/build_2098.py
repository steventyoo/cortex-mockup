#!/usr/bin/env python3
"""
build_2098.py — Build the 17-tab JCR Cortex v2 workbook for Job 2098
(Chinn, Trailside 2) — live/active project.

Input:   2098_data.json  (from parse_2098.py)
Output:  OWP_2098_JCR_Cortex_v2.xlsx
...
DATA_FILE = _HERE / '2098_data.json'
OUT_FILE = _HERE / 'OWP_2098_JCR_Cortex_v2.xlsx'
...
META_2098 = {
    'job_id': '2098',
    'name': 'Trailside Apartment Community Phase II',
    'short_name': 'Trailside 2',
    'gc': 'Chinn Construction, LLC',
    'owner': 'Trailside Ph II SH Holdings, LLC (Dallas, TX)',
    'units': 170,
    'total_fixtures': 1200,
    'insurance': 'Standard (non-OCIP)',
    'job_start': 'Aug 2023',
    ...
    'pay_app_count': 27,
    'cos': [
        {'co': 1, 'desc': 'Pet Wash Budget Reallocation', 'amount': 0},
        {'co': 2, 'desc': 'Gas Meter at Boiler Room', 'amount': 4426},
        {'co': 3, 'desc': 'Condensate Drains at Units', 'amount': 60075},
        {'co': 4, 'desc': 'Lixil Trim Reduction + Sink Upgrades', 'amount': 12975},
        {'co': 5, 'desc': 'OT Premium 12/16/23', 'amount': 1272},
        {'co': 6, 'desc': 'Reduced Cost on Toilets', 'amount': -2055},
        {'co': 7, 'desc': 'Maint. Room Plumbing Fixtures', 'amount': 9500},
        {'co': 8, 'desc': 'Toilet Flange Backcharge', 'amount': -18984.26},
        {'co': 9, 'desc': 'Corrections to Toilet Flange Backcharge', 'amount': 2904},
        {'co': 10, 'desc': 'Shower Stall Backcharges', 'amount': -52104},
        {'co': 11, 'desc': 'Retail Gas & Condensate Changes', 'amount': 5019},
        {'co': 12, 'desc': 'Damaged Toilet Seats', 'amount': 308},
        {'co': 13, 'desc': 'Move Retail Condensate', 'amount': 464},
        {'co': 14, 'desc': 'L2 Amenity Coffee Maker', 'amount': 489},
        {'co': 15, 'desc': 'L2 Amenity Coffee Maker - CANCELLED', 'amount': -489},
        {'co': 16, 'desc': 'Attic Stock', 'amount': 1220},
        {'co': 17, 'desc': 'Boiler Room Cond Drain + Attic Stock Premium', 'amount': 803},
        {'co': 18, 'desc': 'Garage Heat Trace', 'amount': 6263},
    ],
    'pay_apps': [
        # 27 pay apps from Aug 2023 to Nov 2025 — full history in GDrive
    ],
    'change_log': [
        # 17 CORs documented, 3 cancelled (COR#03, COR#06, COR#13-cancelled)
    ],
    'predictive_signals': [
...
def generate_predictive_signals(data):
    ...
    # Retention
    ret = abs(META_2098.get('retention', 0))
    billed = abs(META_2098.get('billed', 0))
...
if __name__ == "__main__":
    data = load_data()
    META_2098['predictive_signals'] = generate_predictive_signals(data)
    out = build_workbook(dict(META_2098))
    print(f"Built: {out}")
