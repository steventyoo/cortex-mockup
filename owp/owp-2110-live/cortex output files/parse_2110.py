#!/usr/bin/env python3
"""
parse_2110.py — Parse Job 2110 JDR PDF → 2110_data.json

Structured parse of the 5-page Sage Timberline Job Detail Report for
Job 2110 (Compass, 3000 Western). Mirrors Cortex v2.2 schema so downstream
build/notionize scripts can consume the same shape.

Customer code: 2110CC · Client: Compass General Construction · GC: Compass.
Project status: CANCELLED (pre-construction, no job activity).
"""
import json
import re
...
# owp-2110-live/cortex output files/ → owp/ → Gemini Accounting Reports/
PDF = _HERE.parent.parent / 'Gemini Accounting Reports' / '2110 Job Detail Report.pdf'
OUT = _HERE / '2110_data.json'

PROJECT_META = {
    'job_number': '2110',
    'job_name': "Roosevelt Manor",
    'general_contractor': "Sierra",
    'owner': "TBD",
    'location': "Roosevelt, Seattle",
    'project_type': "Multi-family apartments",
    'units': 137,
    'total_fixtures': None,
    'architect': 'TBD',
    'mep_engineer': 'TBD',
...
PAGE_HDR_MARKERS = (
    'OWP, LLC', 'Job Detail Report', 'for Job "2110"', 'Job Number: 2110',
    'Job Name: Compass, 3000 Western', 'Legal:',
...
AP2_RE = re.compile(
    r'^(\d{2}/\d{2}/\d{2})\s+(Inv|Credit):\s+(.+?)\s+([\d,]+\.[\d]{2}\-?)(?:\s+([\d,]+\.[\d]{2}\-?))?(?:\s+([\d,]+\.[\d]{2}\-?))?$'
)
...
def main():
    ...
    print(f'  Net profit: ${net_profit:,.2f} ({net_profit/revenue*100:.1f}% if revenue else 0)')
    ...
    data = {
        'schema': 'CORTEX_V2.2',
        'schema_note': 'Parsed direct from 5-page 2110 JDR PDF. Schema matches 2020 gold standard + 2049 v2.2 extensions.',
